...
class Point:
    ...
    def removeFromList(self, pointList: list):
        newPointList = []
        for i in range(len(pointList)):
            if pointList[i].x == self.x and pointList[i].y == self.y:
                continue
            else:
...
class Maze:
    ...
    def put_point_in_to_be_selected(self, point: Point):
        for i in range(len(self.built_x)):
            ...
            if 0 < nxt_point.x < self.row and 0 < nxt_point.y < self.col and self.maze[nxt_point.x][nxt_point.y] == 1 and (not nxt_point.isInList(self.to_be_selected)):
    ...
    def generate_maze(self):
        if len(self.to_be_selected) > 0:
            ...
            self.to_be_selected = select_nodeA.removeFromList(
                self.to_be_selected)
            return False
        else:
            return True
    ...
    def demo(self):
        if len(self.to_be_selected) > 0:
            ...
            self.to_be_selected = select_nodeA.removeFromList(
                self.to_be_selected)
        elif len(self.path_list) > 0:
            self.maze[self.end.x][self.end.y] = 3
            last_corner = self.path_list[-1]
            if last_corner.isSame(self.end):
                return
            for i in range(len(self.run_x)):
                nxt_point = Point(
                    last_corner.x+self.run_x[i], last_corner.y+self.run_y[i])
                if 0 < nxt_point.x < self.row-1 and 0 < nxt_point.y < self.col-1 and (self.maze[nxt_point.x][nxt_point.y] == 0 or self.maze[nxt_point.x][nxt_point.y] == 3) and self.isVisited[nxt_point.x][nxt_point.y] == False:
                    self.isVisited[nxt_point.x][nxt_point.y] = True
                    self.maze[last_corner.x][last_corner.y] = 2
                    self.path_list.append(nxt_point)
                    break
                elif i == 3:
                    self.maze[last_corner.x][last_corner.y] = 0
                    self.path_list.pop()

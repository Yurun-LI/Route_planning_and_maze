...
class Maze:
    ...
    def put_point_in_to_be_selected(self, point: Point):
        for i in range(len(self.built_x)):
            ...
            if 0 < nxt_point.x < self.row and 0 < nxt_point.y < self.col and (nxt_point not in self.to_be_selected) and self.maze[nxt_point.x][nxt_point.y] == 1:
    ...
    def generate_maze(self):
        if len(self.to_be_selected) > 0:
            ...
            print((select_nodeA.x, select_nodeA.y))
            print("before")
            for node in self.to_be_selected:
                print("::", node.x, ':', node.y)
            self.to_be_selected.remove(select_nodeA)
            print("after")
            for node in self.to_be_selected:
                print("::", node.x, ':', node.y)
            print(self.to_be_selected)
